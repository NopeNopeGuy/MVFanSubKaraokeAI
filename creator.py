...
def generate_ass_header(res_x: int, res_y: int, with_translation: bool, is_instrumental_karaoke: bool) -> str:
    ...
    if is_instrumental_karaoke:
        ...
        header += "Style: Karaoke-Upcoming,Arial Black,80,&H00FFFFFF,&H00CCCCCC,&H00000000,&H80000000,-1,0,0,0,100,100,0,0,1,4,2,8,10,10,85,1\n"
        header += "Style: Karaoke-Upcoming2,Arial Black,80,&H00FFFFFF,&H00CCCCCC,&H00000000,&H80000000,-1,0,0,0,100,100,0,0,1,4,2,8,10,10,170,1\n"
        header += "Style: Karaoke-Past,Arial Black,80,&H00FFFFFF,&H00CCCCCC,&H00000000,&H80000000,-1,0,0,0,100,100,0,0,1,4,2,8,10,10,0,1\n"
    else:
...
def create_karaoke_from_structured_json(input_path: str, output_path: str, translate: bool, audio_file_path: str | None, is_instrumental_karaoke: bool, update_json: bool):
    ...
    with open(output_path, 'w', encoding='utf-8-sig') as f:
        ...
        for i, sentence in enumerate(sentences):
            ...
            if is_instrumental_karaoke:
                f.write(f"Dialogue: 0,{start_time_ass},{end_time_ass},Karaoke-Active,,0,0,0,,{{\\pos(960,100)}}{karaoke_text}\n")
                if i > 0:
                    past_text = sentences[i-1].get('text', '')
                    anim_start_s, anim_end_s = start_time_s, start_time_s + 0.5
                    f.write(f"Dialogue: 0,{format_time_ass(anim_start_s)},{format_time_ass(anim_end_s)},Karaoke-Past,,0,0,0,,{{\\move(960,100,960,400)\\alpha&HFF&}}{past_text}\n")
                if i + 1 < len(sentences):
                    next_text = sentences[i+1].get('text', '')
                    f.write(f"Dialogue: 0,{start_time_ass},{end_time_ass},Karaoke-Upcoming,,0,0,0,,{{\\pos(960,185)}}{next_text}\n")
                if i + 2 < len(sentences):
                    next2_text = sentences[i+2].get('text', '')
                    f.write(f"Dialogue: 0,{start_time_ass},{end_time_ass},Karaoke-Upcoming2,,0,0,0,,{{\\pos(960,270)}}{next2_text}\n")
            else:
